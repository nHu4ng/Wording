...
- 导入一份或多份 IELTS 词汇 JSON；每次导入都会追加到「复习文件夹」，不会覆盖此前导入的词表。重复词会合并，同时保留多个来源的追溯信息。
...
每份成功导入的 JSON 都会被复制到网站内的「复习文件夹」；下一次导入会在已有内容上追加，而不是覆盖。该文件夹保存在当前设备、当前浏览器的 IndexedDB 中，因此刷新页面后仍可打开全部词表、单独下载留档文件，或从文件夹移除某一份。

这不是对电脑文件系统的移动操作：原始 JSON 会保留在原位置，不会被网页删除或改名。清除本站的浏览器网站数据、换浏览器或换设备后，复习文件夹不会自动同步；建议同时保留你的原始导出文件。
...
```text
app/
  page.tsx                       # 主界面、追加导入、查询、导出与学习状态
  extended-curated-profiles.ts   # 扩展的已审校 IELTS 词汇档案
  review-folder.ts               # 浏览器 IndexedDB 复习文件夹
  globals.css                    # 页面样式
public/
  og.png                         # 分享预览图
```
...
- 已导入的 JSON 会复制到当前浏览器的 IndexedDB 复习文件夹；学习状态仅写入浏览器 `localStorage`。两者都不会上传到本项目服务器，也不会改动电脑上的原始文件。
